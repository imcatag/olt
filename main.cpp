...
double bestScores[10000];
priority_queue<int, vector<int>, decltype(cmp)> gameQueues[10000];
vector<Node> nodesThr[10000];
int bestNodeIndexes[10000];
...
int main()
{
    ...
    while(true){
        ...
        // start threads
        for(unsigned long long int i = 0; i < nextCount; i++){
            t[i] = thread(generateThr, i, nextNodes[i]);
        }

        // join threads after 0.25 seconds
        while(true){
        ...
        // advance root to best node
        root = nodesThr[bestThread][bestNodeIndexes[bestThread]];

        // print best node

        // add up cntThr
        int cnt = 0;
        for(unsigned long long int i = 0; i < nextNodes.size(); i++){
            cnt += cntThr[i];
        }

        cout << "cnt: " << cnt << "\n";
        cout << "Best node: " << root.score << "\n";
